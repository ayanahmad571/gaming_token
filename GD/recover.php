...
if(isset($_GET['id'])){
	...
	if(ctype_alnum($id)){
		$id = $_GET['id'];
		$tim = time();
		$valchk = getdatafromsql($conn, "select * from sw_recover where concat(rv_hash,rv_hash_2)='".trim($id)."' and rv_used = 0 and rv_valid =1 and rv_valid_till > ".$tim."");
		
		if(is_array($valchk)){
			
			if($conn->query("UPDATE `sw_recover` SET `rv_used`= 1 where
